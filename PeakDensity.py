...
def peak_density_cluster(dist, per, gaussian=False):
    # low = 0.01
    # high = 0.02
    n = len(dist)
    # dc = np.min(dist)
    ...
    # find dc by sort
    nr = 0.015
    pos = np.ceil(n ** 2 * nr + n)
    dc = np.sort(dist.flatten())[pos]
...
if __name__ == '__main__':
    ...
    r, d, pk, cls = peak_density_cluster(distance, 0.3, gaussian=True)
